 ...
 * Everything else (FAQ table, save, delete) is handled by faq_api.php via AJAX.
 */
...
// Courses only — fetched server-side since they don't depend on any selection
$courses = $db->query(
...
<main class="main-content" data-page="faqs">
  ...
  <!-- Result Area — filled by AJAX via faq_api.php -->
  <div id="result-area">
...
function switchFaqTab(btn, cat) {
    ...
    var panel = document.querySelector('.faq-panel[data-cat="' + cat + '"]');
    if (panel) panel.classList.add('active');
}

// ── Save a single FAQ row ────────────────────────────────────────────────────
        ...
        .then(function(data) {
            ...
            if (data.success) {
                ...
                if (data.id && !row.querySelector('.btn-delete-row')) {
                    ...
                    var saveBtn = row.querySelector('.btn-save-row');
                    var delBtn  = document.createElement('button');
                    delBtn.className   = 'btn-delete-row';
                    delBtn.title       = 'Delete';
                    delBtn.textContent = '🗑️';
                    delBtn.setAttribute('onclick', 'deleteFaqRow(this)');
                    saveBtn.insertAdjacentElement('afterend', delBtn);
                }
                updateTabCount(row.dataset.cat);
...
// ── Save all rows in a category tab ─────────────────────────────────────────
...
function updateTabCount(cat) {
    var filled = document.querySelectorAll('#tbody-' + cat + ' tr.has-data').length;
    var tabBtn = document.querySelector('.faq-tab-btn[data-cat="' + cat + '"] .faq-tab-count');
    if (tabBtn) tabBtn.textContent = filled + '/10';
}
</script>
